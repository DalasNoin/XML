...
public class FilmverwaltungGUI2 extends Frame implements ActionListener, Observer {
    ...
    private void onLaden() {
        if (container != null && !saved) {
            ...
            if (fd.getFile() != null) {
                ...
                lblAnzeige.setText(LBL_ANZEIGE_NO_FILE);
        }
    }
    ...
    private void onSpeichernUnter() {
        if (container == null) {
            ...
            FileDialog fd = new FileDialog(this, "Speichern unter", FileDialog.SAVE);
            fd.setVisible(true);
            if (fd.getFile() != null) {
                filename = fd.getDirectory() + fd.getFile();
                save();
